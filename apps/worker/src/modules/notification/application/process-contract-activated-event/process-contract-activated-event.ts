import { NotificationLog } from "@/modules/notification/domain/notification-log";
...
export type { ActivationNotification } from "@/modules/notification/domain/activation-notification";
...
export class ProcessContractActivatedEvent {
  ...
  async execute(event: ActivationNotification): Promise<ProcessingOutcome> {
    try {
      ...
      this.writeFailure(reason, event);
      throw new NotificationProcessingError(reason);
    }
  }

  invalid(payload: unknown): never {
