...
function migrate(
  ...
  return request(app.getHttpServer())
    .post(`/contracts/${contractId}/migrate`)
    .set("Authorization", `Bearer ${token}`)
    .send({ expectedRevision, targetVersionId, values });
}
...
describe("POST /contracts/:id/migrate", () => {
  test("migrates Draft values to the active version with immutable History", async () => {
    if (!cleanupClient) throw new Error("Missing TEST_DATABASE_URL");
    const admin = tokens.get("acme:admin")!;
    const [tenant, globex] = await Promise.all([
    ...
    const logicalTemplate = tenant.logicalTemplate!;
    const sourceVersionId = logicalTemplate.activeVersionId!;
    ...
    try {
      ...
      const unchanged = await migrate(
        admin,
        created.body.id,
        2,
        targetVersionId,
        { title: "After migration", tier: "premium", approved: false },
      );
      const sameVersionChanged = await migrate(
        admin,
        created.body.id,
        2,
        targetVersionId,
        { title: "Changed in place", tier: "standard", approved: false },
      );
      const stale = await migrate(admin, created.body.id, 1, targetVersionId, {
        title: "After migration",
        tier: "premium",
      });
      const invalid = await migrate(
        admin,
        created.body.id,
        3,
        targetVersionId,
      ...
      const foreignTarget = await migrate(
        admin,
        created.body.id,
        3,
        globex.logicalTemplate!.activeVersionId,
        { title: "After migration", tier: "premium" },
      );
      const historyResponse = await history(admin, created.body.id);
      const persisted = await client.contract.findUniqueOrThrow({
        where: { id: created.body.id },
        include: {
          history: { orderBy: { revision: "asc" } },
      ...
      expect({
        changed: {
          ...
          values: changed.body.values,
          templateVersion: changed.body.templateVersion,
        },
        unchanged: [unchanged.status, unchanged.body.revision],
        sameVersionChanged: [
          sameVersionChanged.status,
          sameVersionChanged.body.revision,
          sameVersionChanged.body.values,
        ],
        stale: [stale.status, stale.body.code],
        invalid: [invalid.status, invalid.body.code, invalid.body.issues],
        member: member.status,
        foreignTarget: [foreignTarget.status, foreignTarget.body.code],
        persisted: {
          revision: persisted.revision,
        ...
        history: historyResponse.body.entries.map(
          (entry: {
            action: string;
            before: { templateVersion: { id: string } } | null;
            after: { templateVersion: { id: string } };
          }) => ({
            action: entry.action,
            beforeVersion: entry.before?.templateVersion.id ?? null,
            afterVersion: entry.after.templateVersion.id,
          }),
        ),
      }).toStrictEqual({
        changed: {
        ...
        sameVersionChanged: [
          200,
          3,
          { title: "Changed in place", approved: false, tier: "standard" },
        ],
        stale: [409, "CONTRACT_REVISION_CONFLICT"],
        invalid: [
          400,
          "INVALID_CONTRACT_VALUES",
          [
            { key: "tier", code: "REQUIRED" },
            { key: "effective-date", code: "UNKNOWN_FIELD" },
          ],
        ],
        member: 403,
        foreignTarget: [404, "TEMPLATE_VERSION_NOT_FOUND"],
        persisted: {
          revision: 3,
          ...
          actions: ["CREATED", "MIGRATED", "MIGRATED"],
          outbox: 0,
        },
        history: [
          {
            action: "CREATED",
            beforeVersion: null,
            afterVersion: sourceVersionId,
          },
          {
            action: "MIGRATED",
            beforeVersion: sourceVersionId,
            afterVersion: targetVersionId,
          },
          {
            action: "MIGRATED",
            beforeVersion: targetVersionId,
            afterVersion: targetVersionId,
          },
        ],
      });
      ...
      try {
        await cleanupClient.contractHistory.deleteMany({
          where: { contractId: created.body.id },
        });
      } finally {
        await cleanupClient.$executeRawUnsafe(
      ...
      await client.contract.delete({ where: { id: created.body.id } });
      await client.logicalTemplate.update({
        where: { id: logicalTemplate.id },
        data: { activeVersionId: sourceVersionId },
      });
      await removeTemplateVersionFixture(targetVersionId);
    }
  });
});
